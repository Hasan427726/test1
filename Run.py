...
\t   [underline blue]Coded by Rozhak-XD""", width=47, title="Version 2.5", style="bold white")) # Coded by Rozhak-XD
    try:
        ...
        if token.lower() == 'get' or 'get' in str(token.lower()):
            ...
            try:
                cookies = {}
                for key in requests.get('https://api.facebook.com/method/auth.={}&format=json&new_app_id=1348564698517390&generate_session_cookies=1'.format(token)).json()['session_cookies']:
...
[bold white][[bold green]4[bold white]]. Keluar ([bold red]Exit[bold white])""", width=47, style="bold white"))
    try:
        ...
        print(Panel(f"[bold red] {str(e).title()}!", width=47, style="bold white"));break7, style="bold white"));break